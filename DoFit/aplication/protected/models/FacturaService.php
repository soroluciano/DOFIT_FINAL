...
class FacturaService
{
	...
	public function GenerarFactura($actividad,$usuario,$anio,$mes){
        ...
        $pdf->Ln(8);
        $pdf->Cell(95,10,$nombreGimnasio,0,1,'C');
        $pdf->Cell(95,5,$direccionGimnasio,0,1,'C');
        $pdf->Cell(95,5,$localidadGimnasio.' '.$provinciaGimnasio.' '.$telefonoGimnasio,0,1,'C');
        $pdf->Cell(95,5,'I.V.A. Exento',0,1,'C');
        $pdf->SetXY(110,10);
        $pdf->Cell(95,10,'VOLANTE DE PAGO',0,1,'L');
        $pdf->SetXY(110,15);
        $pdf->Cell(95,10,'Nº '.$actividad.$usuario.$anio.$mes,0,1,'L');
        $pdf->SetXY(80,50);
        $pdf->Image( Yii::app()->request->baseUrl.'/img/logo.png', 40, 10, 40, 15);
        $pdf->SetXY(115,25);
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(95,5,'FECHA:',0,1,'L');
        $pdf->SetXY(160,25);
        $pdf->Cell(95,5,date('d/m/y'),0,1,'L');
        $pdf->SetXY(115,30);
        $pdf->Cell(95,5,'C.U.I.T:',0,1,'L');
        $pdf->SetXY(160,30);
        $pdf->Cell(95,5,$cuitGimnasio,0,1,'L');
        $pdf->SetXY(115,35);
        ...
        $pdf->SetXY(160,40);
        $pdf->Cell(95,5,'01/01/2016',0,1,'L');
        $pdf->Rect(10, 50, 190, 40);
        $pdf->SetFont('Arial','',9);
        $pdf->SetXY(17,50);
        $pdf->Cell(95,10,'Señores:',0,1,'L');
        $pdf->SetXY(40,50);
        $pdf->Cell(95,10,$nombreAlumno.' '.$apellidoAlumno,0,1,'L');
        $pdf->SetXY(17,60);
        $pdf->Cell(95,10,'Domicilio:',0,1,'L');
        $pdf->SetXY(40,60);
        $pdf->Cell(95,10,$direccionAlumno,0,1,'L');
        $pdf->SetXY(17,70);
        $pdf->Cell(95,10,'Localidad:',0,1,'L');
        $pdf->SetXY(40,70);
        $pdf->Cell(95,10,$localidadAlumno,0,1,'L');
        $pdf->SetXY(17,80);
        $pdf->Cell(95,10,'Provincia:',0,1,'L');
        $pdf->SetXY(40,80);
        $pdf->Cell(95,10,$provinciaAlumno,0,1,'L');
        $pdf->Ln(5);
        ...
        $pdf->SetXY(10,105);
        $pdf->Cell($w[0],10,'1',0,1,'C');
        $pdf->SetXY(40,105);
        $pdf->Cell($w[1],10,$deporte,0,1,'L');
        $pdf->SetXY(140,105);
        $pdf->Cell($w[2],10,$precio,0,1,'C');
        $pdf->SetXY(170,105);
        $pdf->Cell($w[2],10,$precio,0,1,'C');
